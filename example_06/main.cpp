...
template <>
struct is_message<legacy_messages::MessageA> : std::true_type {};

template <>
struct is_message<legacy_messages::MessageB> : std::true_type {};

template <>
struct is_message<legacy_messages::MessageC> : std::true_type {};

template <>
struct is_message<legacy_messages::MessageD> : std::true_type {};

template <typename Message>
std::enable_if_t<is_message_v<Message>, decltype(Message::header)> getHeader(
    const Message& message) {
  return message.header;
}

template <typename Message>
std::enable_if_t<is_message_v<Message>, decltype(Message::hdr)> getHeader(
    const Message& message) {
  return message.hdr;
}

template <typename Message>
std::enable_if_t<is_message_v<Message>, decltype(Message::description)> getHeader(
    const Message& message) {
  return message.description;
}

template <class Message>
typename std::enable_if_t<is_message<Message>::value, void> printTimestamp(
    const Message& message) {
...
int main() {
  ...
  std::cout << "is_message(c) = "
            << is_message<decltype(c)>::value << " : ";
  printTimestamp(c);

  std::cout << "is_message(d) = "
            << is_message<decltype(d)>::value << " : ";
  printTimestamp(d);
